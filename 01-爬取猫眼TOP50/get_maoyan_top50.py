...
# 利用 request 来简单爬取网页的关键点
# 1. 分析网站的 request 要求,如需要代理,需要登录密码,需要验证
# 2. 构建出合适的正则表达式,这里有一个技巧,利用[标志位]来快速定位所需要提取的内容位置.一般选取目标内容左边的属性名作为标志位
# 3. 将经过正则表达式解析过的内容写入文件,或是加入数据库中,持久化内容,但是要考虑内存使用.
# 4. 一般解析过后的内容都处理为 JSON 格式,利用 JSON.dumps() 使得字典序列化,方便写入文件,另外写入文件时,使用[a]追加模式.
